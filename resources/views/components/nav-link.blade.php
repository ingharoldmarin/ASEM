@props(['href', 'label', 'icon' => 'home'])

@php
$active = str_starts_with(request()->url(), $href) || request()->url() === $href;
@endphp

<a href="{{ $href }}"
   class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm transition-colors font-medium"
   style="{{ $active
        ? 'background-color:#39A900;color:#ffffff;'
        : 'color:rgba(255,255,255,0.70);' }}"
   onmouseover="{{ $active ? '' : "this.style.backgroundColor='rgba(57,169,0,0.18)';this.style.color='#fff';" }}"
   onmouseout="{{ $active ? '' : "this.style.backgroundColor='';this.style.color='rgba(255,255,255,0.70)';" }}">
